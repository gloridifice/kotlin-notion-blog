...
class ArticleContent(val markdownText: String, val filePath: Path) {
            ...
            MarkdownElementTypes.BLOCK_QUOTE -> {
                blockQuote {
                    for (child in node.children) {
                        if (child.type != MarkdownTokenTypes.BLOCK_QUOTE && child.type != MarkdownTokenTypes.Companion.EOL) {
                            markdownBlock(child)
                        }
                    }
                }
    ...
    private fun FlowContent.renderInlineNode(node: ASTNode) {
        when (node.type) {
            ...
            MarkdownTokenTypes.Companion.ATX_CONTENT -> {
            ...
            MarkdownTokenTypes.Companion.ATX_HEADER -> {
                // do nothing
            }

            MarkdownTokenTypes.Companion.CODE_LINE -> +node.getTextInNode(markdownText).toString()

            MarkdownTokenTypes.Companion.HARD_LINE_BREAK -> br
    ...
    private fun FlowContent.renderCodeFence(node: ASTNode) {
        val lang = node.children
            .find { it.type == MarkdownTokenTypes.Companion.FENCE_LANG }
            ?.getTextInNode(markdownText)?.toString()
        val codeLines = node.children
            .filter {
                it.type == MarkdownTokenTypes.Companion.CODE_FENCE_CONTENT ||
                        it.type == MarkdownTokenTypes.Companion.CODE_LINE
            }
    ...
    private fun FlowContent.renderCodeBlock(node: ASTNode) {
        val codeLines = node.children
            .filter { it.type == MarkdownTokenTypes.Companion.CODE_LINE }
            .joinToString("\n") { it.getTextInNode(markdownText).toString() }
            .replace("\t", "  ")
        div {
            classes += "code_block"
